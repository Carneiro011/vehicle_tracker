...
class AreaSelector:
    def __init__(self, window_name="Definir Areas"):
        ...
        # areas[0] = entrada, areas[1] = saída (opcional)
        self.areas = [[], []]
    ...
    def _mouse_callback(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN and self.highlighted_point_index != -1:
            # clicar e arrastar ponto existente
            ...
            # finalizar área atual
            root = self._create_tk_root()
            if self.current_area_index == 0:
                if messagebox.askyesno("Área de Saída",
                                       "Deseja definir uma área de SAÍDA também?",
                                       parent=root):
                    self.current_area_index = 1
                else:
                    messagebox.showinfo("Pronto",
                                        "Área de ENTRADA definida. Pressione 'S' para salvar.",
                                        parent=root)
            else:
                messagebox.showinfo("Concluído",
                                    "Área de SAÍDA definida. Pressione 'S' para salvar.",
                                    parent=root)
            root.destroy()
            self._refocus_window()

        # mover ponto enquanto arrasta
    ...
    def _draw(self):
        ...
        for i, area_points in enumerate(self.areas):
            ...
            cv2.fillPoly(overlay, [np.array(area_points, dtype=np.int32)], color)
            cv2.addWeighted(overlay, 0.3, self.display_frame, 0.7, 0, self.display_frame)
            cv2.polylines(self.display_frame, [np.array(area_points, dtype=np.int32)],
                          True, color, 2)
            for j, p in enumerate(area_points):
                pt_color = (self.COLORS["highlight"]
                            if j == self.highlighted_point_index and i == self.current_area_index
                            else color)
    ...
    def _draw_ui(self):
        area_name = "ENTRADA" if self.current_area_index == 0 else "SAÍDA"
        color = self.COLORS["entry"] if self.current_area_index == 0 else self.COLORS["exit"]
        cv2.putText(self.display_frame, f"Definindo Área: {area_name}",
                    (20, 40), self.FONT, 1, color, 2)
        instructions = [
            "Clique ESQUERDO para adicionar/arrastar pontos.",
            "Clique DIREITO para finalizar a área atual.",
            "Z: Reiniciar área ATUAL | R: Reiniciar TUDO",
            "S: Salvar e Sair | ESC: Sair sem Salvar"
        ]
        for i, text in enumerate(instructions):
            cv2.putText(self.display_frame, text,
                        (20, self.WINDOW_HEIGHT - 100 + i*25),
    ...
    def _load_areas(self):
        ...
        if os.path.exists(path):
            root = self._create_tk_root()
            if messagebox.askyesno("Carregar Áreas",
                                   "Arquivo 'areas.json' encontrado. Deseja editar?",
                                   parent=root):
                with open(path, 'r') as f:
                    loaded = json.load(f)
                sx = self.WINDOW_WIDTH  / self.original_dims[0]
                sy = self.WINDOW_HEIGHT / self.original_dims[1]
                self.areas[0] = [[int(p[0]*sx), int(p[1]*sy)] for p in loaded[0]]
                if len(loaded) > 1:
                    self.areas[1] = [[int(p[0]*sx), int(p[1]*sy)] for p in loaded[1]]
                root.destroy()
                self._refocus_window()
                return True
            root.destroy()
    ...
    def _save_areas(self):
        ...
        if not self.areas[0]:
            messagebox.showwarning(
                "Aviso",
                "A área de ENTRADA precisa ser definida para salvar.",
                parent=root
            )
            root.destroy()
            self._refocus_window()
        ...
        entry_scaled = [[int(p[0]*sx), int(p[1]*sy)] for p in self.areas[0]]
        scaled_areas = [entry_scaled]
        # só adiciona área de saída se tiver 3+ pontos
        if len(self.areas[1]) >= 3:
            exit_scaled = [[int(p[0]*sx), int(p[1]*sy)] for p in self.areas[1]]
            scaled_areas.append(exit_scaled)
        ...
        msg = f"Área(s) salva(s) em:\n{os.path.abspath(path)}"
        if len(scaled_areas) == 1:
    ...
    def run(self, video_source=None):
        ...
        cap.release()
        self.original_frame = cv2.resize(frame,
        ...
        while True:
            if cv2.getWindowProperty(self.window_name, cv2.WND_PROP_VISIBLE) < 1:
                root = self._create_tk_root()
                if messagebox.askyesno("Sair", "Sair sem salvar?", parent=root):
                    root.destroy()
                    break
                root.destroy()
                self._refocus_window()
            ...
            if key == 27:  # ESC
                ...
                root = self._create_tk_root()
                if messagebox.askyesno("Reiniciar Tudo", "Apagar todas as áreas?", parent=root):
                    self.areas = [[], []]
                    self.current_area_index = 0
                root.destroy()
                self._refocus_window()
